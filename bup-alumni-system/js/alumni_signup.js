/**
 * BUP Alumni System - Alumni Signup JavaScript
...
document.getElementById('alumniSignupForm').addEventListener('submit', async function(e) {
    e.preventDefault();
...
function collectFormData() {
    return {
        ...
        alumniStatus: document.getElementById('alumniStatus').value,
        address: document.getElementById('address').value.trim(),
        religion: document.getElementById('religion').value,
        blood_group: document.getElementById('bloodgroup').value,
        gender: getRadioValue('gender'),
        currentWorkplace: document.getElementById('currentWorkplace').value.trim(),
        profession: document.getElementById('designation').value.trim(), // Using designation as profession
...
function validateFormData(formData) {
    ...
    if (!formData.alumniStatus) errors.push('Alumni status is required');
    ...
    if (!formData.blood_group) errors.push('Blood group is required');
...
function storeFormData() {
    ...
    localStorage.setItem('alumni_signup_data', JSON.stringify(formData));
}

function restoreFormData() {
    const stored = localStorage.getItem('alumni_signup_data');
    if (stored) {
        ...
        if (Date.now() - formData.timestamp < 24 * 60 * 60 * 1000) {
            ...
            document.getElementById('graduationYear').value = formData.graduationYear || '';
            document.getElementById('alumniStatus').value = formData.alumniStatus || '';
            ...
            document.getElementById('currentWorkplace').value = formData.currentWorkplace || '';
...
function clearStoredFormData() {
    localStorage.removeItem('alumni_signup_data');
}
...
async function registerAlumni(formData) {
    console.log('🌐 Making API call to:', `${API_BASE_URL}/alumni/register`);
...
// Initialize batch and graduation year dropdowns
document.addEventListener('DOMContentLoaded', function() {
    const batchSelect = document.getElementById('batch');
    const graduationYearSelect = document.getElementById('graduationYear');
    const currentYear = new Date().getFullYear();
    
    // Initialize batch dropdown (2008 to current year)
    ...
    // Initialize graduation year dropdown (2012 to current year + 1)
    ...
    console.log('✅ Alumni signup page loaded successfully');
